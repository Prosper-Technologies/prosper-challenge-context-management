...
Counts: ~30 locations · ~150 providers · ~82 appointment types · ~300 weekly availability blocks.
...
```jsonc
{
  ...
  "schedules": [{                     // recurring weekly availability (no stale dates)
    "id", "provider_id", "location_id", "weekday", "start", "end"
  }]
}
```
...
- **Duplicate provider names** — multiple `Dr. Chen`, several `Dr. Maria Garcia` across
...
- **Location-gated services** — only a few locations have `imaging`, so a provider may
  practice at 3 sites but offer an MRI at only one.
- **Multi-location providers** — "book with Dr. X" still needs a location to disambiguate.

## Regenerating / scaling

The dataset is generated deterministically (`seed = 7`) by `generate_catalog.py`. Bump
`N_LOCATIONS` / `N_PROVIDERS` in that script and re-run it to push toward the "hundreds of
locations, thousands of appointment types" scale the challenge asks about:

```bash
python3 data/generate_catalog.py data/catalog.json
```
